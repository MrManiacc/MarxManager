...
import marx.plugin.platform.generation.ui.EventGenerationDialog
import marx.plugin.resource.Localize
import marx.plugin.utils.castNotNull
 ...
 * [.cleanup] to complete
 */
class GenerateEventListenerHandler : GenerateMembersHandlerBase(Localize("generate.event_listener.title")) {
    ...
    override fun chooseOriginalMembers(aClass: PsiClass, project: Project, editor: Editor): Array<ClassMember>? {
        ...
        val facet = MarxFacet(moduleForPsiElement) ?: return null
    ...
    private fun isSuperEventListenerAllowed(eventClass: PsiClass, facet: MarxFacet): Boolean {
        val supers = eventClass.supers
        for (aSuper in supers) {
